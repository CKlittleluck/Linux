...
class ChatServer
{
    public:
        ...
        int Start()
        {
            ...
            while(1)
            {
                ...
                pthread_t tid;
                int ret = pthread_create(&tid, NULL, LoginRegisterStar, (void*)tc);
                if(ret < 0)
                {
                    close(new_sock);
                    delete tc;
    ...
    private:
        ...
        static void* LoginRegisterStar(void* arg)
        {
            pthread_detach(pthread_self());
    ...
    private:
        //不管是注册成功了, 还是注册失败了, 都需要给客户端返回一个应答
            ...
            int RecvMsg()
            {
                 ...
                 *       如果不是: 说明这个用户就是老用户了, 之前已经保存过该用户的udp地址信息  
                 *4.将数据发送到消息池当中
                 */
                ...
                ssize_t recv_szie = recvfrom(udp_sock_, buf, sizeof(buf) - 1, 0, (struct sockaddr*)&peer_addr, &peer_addr_len);
